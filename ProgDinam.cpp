...
int MinSinPintarProgDinamAux(std::vector<int>& ent, std::vector<std::vector<std::vector<int> > >& cubo, int i, int n, int ultRo, int ultAz){
	...
	if(ultRo > n-1 || (ent[i] < ent[ultRo])){		// si se puede pintar de rojo, calculo el mínimo pintando ese de rojo
		c = MinSinPintarProgDinamAux(ent, cubo, i-1, n, i, ultAz);
	}
	if(ultAz > n-1 || ent[i] > ent[ultAz]){			// si se puede pintar de azul, calculo el mínimo pintándolo de azul
		v = MinSinPintarProgDinamAux(ent, cubo, i-1, n, ultRo, i);
	}
	b = MinSinPintarProgDinamAux(ent, cubo, i-1, n, ultRo, ultAz) + 1;		// calculo el mínimo no pintándolo
...
int MinSinPintarProgDinam(std::vector<int>& ent, int n){
	...
	int r = n;
	int a = n;
	int res = MinSinPintarProgDinamAux(ent, cube, n-1, n, r, a);
	// cout << res << endl;
	return res;
}
